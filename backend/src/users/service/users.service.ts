...
export class UsersService {
  ...
  async getUserProducts(cpf: string) {
    const query = `SELECT p.id, p.nome, p.marca, p.valor, p.estoque FROM produto p, cadastra c WHERE (c.id_produto, cpf_gerente) = (p.id, '${cpf}') ORDER BY p.id`;
    const products: Product[] = await this.userRepository.query(query);
    return products;
  }

  async getUserMonitoringProducts(cpf: string) {
    const query = `SELECT p.id, p.nome, p.marca, p.valor, p.estoque, m.data_inicio, m.data_termino, m.expectativa_vendas, m.vendas_realizadas FROM produto p, monitora m WHERE (m.id_produto, cpf_gerente) = (p.id, '${cpf}') ORDER BY p.id`;
